...
//欧拉角转换到旋转矩阵
Eigen::Matrix3d euler2RotationMatrix(const double roll, const double pitch,
                                     const double yaw) {
  Eigen::AngleAxisd rollAngle(roll, Eigen::Vector3d::UnitZ());
  Eigen::AngleAxisd yawAngle(yaw, Eigen::Vector3d::UnitY());
  Eigen::AngleAxisd pitchAngle(pitch, Eigen::Vector3d::UnitX());
...
//////////////////////////////初始-抓取////////////////////////////////////////////
//接收数据点
void ROI_pointcentre_Callback(
    const std_msgs::Float32MultiArray::ConstPtr &array) {
  int i = 0, j = 0;
  float msgs[100];
...
void transformPoint(const tf::TransformListener &listener) {
  ...
  if (getClassJudge) {
    try {
      ...
      ROS_INFO("base_camrea: (%.2f, %.2f. %.2f) -----> base_hand: (%.2f, %.2f, "
               "%.2f) at time %.2f",
               camera_point.point.x, camera_point.point.y, camera_point.point.z,
               hand_point.point.x, hand_point.point.y, hand_point.point.z,
               hand_point.header.stamp.toSec());
      getClassPoint = true;
    } catch (tf::TransformException &ex) {
      ROS_ERROR("Received an exception trying to transform a point from "
                "\"base_camrea\" to \"base_hand\": %s",
                ex.what());
    }
    getClassJudge = false;
  }
...
void stateCatch(moveit::planning_interface::MoveGroupInterface &group) {
  ...
  ROS_INFO("Visualizing plan 1_1 (stateCatch pose) %s",
  ...
  ROS_INFO("Visualizing plan 1_2 (stateCatch pose) %s",
           success == moveit_msgs::MoveItErrorCodes::SUCCESS ? "SUCCESS"
                                                             : "FAILED");
  if (success == moveit_msgs::MoveItErrorCodes::SUCCESS) {
...
int main(int argc, char **argv) {
  ...
  // ac("six_hand_controller/follow_joint_trajectory", true);
