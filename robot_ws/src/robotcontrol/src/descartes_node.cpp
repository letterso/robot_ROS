#include "robotcontrol/PlanCartesianPath.h"
#include <ros/ros.h>
...
std::vector<double> getCurrentJointState(const std::string &topic) {
  sensor_msgs::JointStateConstPtr state =
      ros::topic::waitForMessage<sensor_msgs::JointState>(topic,
                                                          ros::Duration(0.0));
  if (!state)
...
EigenSTL::vector_Affine3d makeLine(const Eigen::Vector3d &start,
                                   const Eigen::Vector3d &stop, double ds) {
...
public:
  CartesianPlanner(ros::NodeHandle &nh) {
    ...
    // init services
    server_ =
        nh.advertiseService("plan_path", &CartesianPlanner::planPath, this);
  }
  ...
  bool initDescartes() {
    ...
    const std::string group_name = "hand";
  ...
  bool planPath(robotcontrol::PlanCartesianPathRequest &req,
                robotcontrol::PlanCartesianPathResponse &res) {
    ROS_INFO("Recieved cartesian planning request");

    // Step 1: Generate path poses
    EigenSTL::vector_Affine3d tool_poses = makeToolPoses();

    // Step 2: Translate that path by the input reference pose and convert to
    // "Descartes points"
    std::vector<descartes_core::TrajectoryPtPtr> path =
        makeDescartesTrajectory(req.pose, tool_poses);
    ...
    descartes_core::TrajectoryPtPtr pt(
        new descartes_trajectory::JointTrajectoryPt(start_joints));
    path.front() = pt;
    ...
    descartes_utilities::toRosJointPoints(*model, result, 1.0,
  ...
  EigenSTL::vector_Affine3d makeToolPoses() {
    ...
    // NOTE that the makeLine function will create a sequence of points
    // inclusive
  ...
  makeDescartesTrajectory(const geometry_msgs::Pose &reference,
    ...
    for (auto &point : path) {
      ...
      descartes_core::TrajectoryPtPtr pt =
          makeTolerancedCartesianPoint(ref * point);
  ...
  makeTolerancedCartesianPoint(const Eigen::Affine3d &pose) {
    ...
    return TrajectoryPtPtr(
        new AxialSymmetricPt(pose, M_PI / 2.0, AxialSymmetricPt::Z_AXIS));
  }
