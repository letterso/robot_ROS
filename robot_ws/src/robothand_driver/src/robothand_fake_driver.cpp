...
public:
  FollowJointTrajectoryAction(std::string name)
    ...
    judge = false;
    robot_state_pub =
        nh_.advertise<sensor_msgs::JointState>("fake/robot_state", 100);
    // as_.registerGoalCallback(boost::bind(&FollowJointTrajectoryAction::goalCB,
    // this, _1));
    as_.registerGoalCallback(
        boost::bind(&FollowJointTrajectoryAction::goalCB, this));
    as_.registerPreemptCallback(
        boost::bind(&FollowJointTrajectoryAction::preemptCB, this));
    as_.start();
  ...
  // get private data
  int *getdata() { return pos_int; }

public:
  bool judge;
  bool finish() { return judge; }

  void finish_n() { judge = false; }

public:
  int sen_length;
  int get_length() { return sen_length; }
  ...
  void goalCB() {
    ...
    if (as_.isNewGoalAvailable()) {
      fake_robot_state.name = as_.acceptNewGoal()->trajectory.joint_names;
      fake_robot_state.header = as_.acceptNewGoal()->trajectory.header;
      data_time =  as_.acceptNewGoal()->trajectory.header.stamp;
      ...
      for (int i = 0; i < points_->size(); i++) {
        fake_robot_state.header.stamp = data_time + points_->at(i).time_from_start;
        fake_robot_state.position = points_->at(i).positions;
        fake_robot_state.velocity = points_->at(i).velocities;
        fake_robot_state.effort = points_->at(i).accelerations;
      }
      robot_state_pub.publish(fake_robot_state);
    } else {
...
int main(int argc, char **argv) {
  ...
  FollowJointTrajectoryAction followJointTrajectoryAction(
      "six_hand_controller/follow_joint_trajectory");
  // serial
  /* ros::NodeHandle n_("~");
...
*/
  ...
  ros::Subscriber robotcontrol_sub = nh_.subscribe<std_msgs::Int16>(
      "/robot/control", 10, robotcontrol_Callback);
    ...
    if (robotcontroldata == 2) {
      ...
      if (length_n > Max_data_length) {
        ...
        for (i = 0; i < (Max_data_length / 6); i++) {
          ...
          pos_int_adjust[i * 6 + 1] =
              pointdata[(i * 6 * length_quotient + 1) + j * 6];
          pos_int_adjust[i * 6 + 2] =
              pointdata[(i * 6 * length_quotient + 2) + j * 6];
          pos_int_adjust[i * 6 + 3] =
              pointdata[(i * 6 * length_quotient + 3) + j * 6];
          pos_int_adjust[i * 6 + 4] =
              pointdata[(i * 6 * length_quotient + 4) + j * 6];
          pos_int_adjust[i * 6 + 5] =
              pointdata[(i * 6 * length_quotient + 5) + j * 6];
